...
### [BUG-013] Chart rows duplicate on `/sync` across AIRAC editions
- **Symptom**: After clicking „Daten von DFS aktualisieren" on a detail page that had been previously imported under an older AIRAC cycle, every chart appears twice — once with the old `source_airac_cycle` and once with the new one. UI thumbnail grid doubles; print workflows produce 2× the expected pages. EDDF: 12 rows instead of 6.
- **Root cause**: `services/data-ingestion/app/services/manifest_import.py` deduplicated incoming chart entries via the DB `source_url` column. That URL embeds the AIRAC edition slug (`/BasicVFR/2026MAY05/pages/...`). When the edition changed, the lookup missed the previous edition's row, the new row was inserted, and the old row stayed behind. Runways and frequencies were not affected because the extraction runner truncates them per group before re-inserting.
- **Fix**: (a) Match incoming charts by `(aerodrome_icao, title)` — `title` is the stable DFS document name, which doesn't change across editions. On match, update the row in place (`source_url`, `local_path`, `source_airac_cycle`, classification) while **preserving** `rotation_degrees` (user-edited). Charts present in DB but absent from the freshly scraped manifest are deleted (DFS removed them). (b) One-off alembic migration `20260518_0001_dedupe_charts_per_aerodrome` cleans up existing duplicates: for each `(icao, title)` group, the row with the highest `source_airac_cycle` survives and inherits the highest non-zero `rotation_degrees` from any sibling.
- **Verification**: 1 042 → 1 042 charts post-migration (was bloated by duplicates), every `(icao, title)` group of size 1. Re-syncing EDDF afterwards stays at 6; setting `rotation_degrees=90` on a chart and re-syncing keeps the value on the same row id (in-place update, not delete+insert).
- **Affected files**: `services/data-ingestion/app/services/manifest_import.py`, `services/data-ingestion/alembic/versions/20260518_0001_dedupe_charts_per_aerodrome.py` (new)
- **Commit/PR**: #73 → fix branch `fix/73-sync-duplicate-charts`
...
- **Compounding factor**: The `frequencies_v2.md` prompt mentioned bilingual labels as a layout variant but never instructed the LLM to *normalize* to the English canonical token. With consensus keying on the exact type string, any mismatch between „twr" (Claude) and „turm" (OpenAI) would also drop the entry.
- **Fix**: (a) Extend `_FREQ_TYPE_ALIASES` with German aliases (`turm/boden/rollkontrolle/anflug/abflug/freigabe/funk/info/notfrequenz/notruf`) and a recursive split for compound forms (`tower/turm` → first half wins). VDF maps to TWR (shared frequency). (b) New prompt `frequencies_v3.md` makes the "always emit English token" rule the very first absolute rule, lists the German→English mapping table inline, adds Hof-Plauen as worked example, and explicitly demands every visible frequency be extracted.
- **Verification**: EDQM re-extraction now writes both `TWR 124.355 (HOF TOWER / HOF TURM)` and `FIS 125.800`. Spot check on 5 random EDxx places: 2 fully agree (RADIO+FIS), 3 still `no_consensus` — those are an OCR-precision issue (different decimal readings between providers), not the same root cause. Tracked separately.
- **Affected files**: `services/data-ingestion/app/services/extraction_runner.py`, `services/data-ingestion/app/parser/prompts/frequencies_v3.md` (new), `services/data-ingestion/tests/test_extraction_runner_helpers.py`
- **Commit/PR**: #64 → fix branch `fix/64-frequency-extraction-twr`
